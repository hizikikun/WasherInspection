...
class FinalResinWasherDataset:
    """最終版樹脂製ワッシャーデータセット管理クラス"""
    ...
    def __init__(self, data_dir="cs AI学習データ/樹脂"):
        ...
        self.labels = []
        self.defect_types = []
        self.image_size = (128, 128)  # より小さなサイズで高速化
        ...
        for category, label in self.defect_categories.items():
            ...
            for img_path in unique_files:
                try:
                    ...
                    if img is not None:
                        # 最適化された前処理
                        img = self._optimized_preprocessing(img)
    ...
    def _optimized_preprocessing(self, img):
        """最適化された前処理"""
        # リサイズ
    ...
    def create_balanced_dataset(self, target_samples_per_class=150):
        """バランスの取れたデータセットを作成"""
        ...
        for category, label in self.defect_categories.items():
            ...
            if current_count >= target_samples_per_class:
                ...
                # データ拡張で不足分を補完
                augmented_images = self._smart_augmentation(class_images, needed)
    ...
    def _smart_augmentation(self, images, target_count):
        """スマートなデータ拡張"""
        augmented = []
...
def train_final_model():
    """最終版モデルを学習"""
    print("=== 最終版樹脂製ワッシャー分類モデル学習開始 ===")
    
    # データセット読み込み
    dataset = FinalResinWasherDataset()
    images, labels = dataset.load_images()
    
    # バランス調整
    dataset.create_balanced_dataset(target_samples_per_class=150)
    ...
    # モデル構築
    model_builder = FinalResinWasherModel()
