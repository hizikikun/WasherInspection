...
class ImprovedMultiCameraSelectionSystem:
        ...
        for cam_id in range(10):  # Check cameras 0-9
            ...
            if cap.isOpened():
                ...
                if ret and frame is not None:
                    ...
                    # Special handling for Camera 2 (C920n プロ HD webcam)
                    if cam_id == 2:
                        print(f"    Camera 2 detected (C920n プロ HD webcam) - applying advanced Full HD settings...")
                        
                        # Release current capture and reinitialize with specific backend
                        cap.release()
                        
                        # Try different backends for C920n プロ HD
    ...
    def show_improved_camera_selection(self):
        ...
        while selected_camera is None:
            ...
            try:
                window_visible = cv2.getWindowプロperty(selection_window, cv2.WND_PROP_VISIBLE)
                if window_visible < 1:
    ...
    def initialize_selected_camera(self, selected_camera):
        ...
        if ret and test_frame is not None:
            ...
            # Enable Full HD for Camera 2 (C920n プロ HD) and Camera 3
            if self.camera_id == 2:
                print("Enabling Camera 2 (C920n プロ HD) Full HD resolution...")
                # Additional C920n プロ HD specific settings
    ...
    def detect_washer(self, frame):
        """高品質-robust washer detection using deep learning + traditional methods"""
        if frame is None:
            return False, 0.0, []
        ...
        try:
            ...
            # Method 3: 高度な Hough Circle Detection with Multiple Parameters
            circles_detected = []
    ...
    def predict_defect(self, frame):
        ...
        # 改良 decision logic - improved accuracy
    ...
    def run(self):
        ...
        while True:
            ...
            # Check for window close (X button) - enhanced detection
            try:
                window_visible = cv2.getWindowプロperty(self.window_title, cv2.WND_PROP_VISIBLE)
                if window_visible < 1:
