...
def auto_accept_execution():
    ...
    # プロセス監視とAccept自動化
    Register-WmiEvent -Query "SELECT * FROM Win32_プロcessStartTrace WHERE プロcessName='python.exe'" -Action {
        Start-Sleep -Seconds 1
        $process = Get-プロcess python -ErrorAction SilentlyContinue
        if ($process) {
            # Accept自動化（仮想的なキー入力）
...
if __name__ == "__main__":
    ...
    inspection = 完璧なFocusResinInspection(camera_id)
    inspection.run()