...
def run(args):
    ...
    # ノートPC検出と軽量化設定
    is_notebook = False
    frame_skip_interval = 1  # デフォルト: スキップなし
    wait_key_delay = 1  # デフォルト: 1ms
    processing_resolution = None  # デフォルト: フル解像度
    ...
    try:
        ...
        # ノートPC検出
        is_notebook = detector.specs.get('is_notebook', False)
        if is_notebook:
            print("\n[軽量モード] ノートPCが検出されました。軽量モードで動作します。")
            frame_skip_interval = 3  # 3フレームごとに1フレーム処理（フリーズ防止）
            wait_key_delay = 30  # 30ms待機（CPU負荷軽減）
            processing_resolution = (640, 480)  # 低解像度で処理
            print(f"  → フレームスキップ: {frame_skip_interval}フレームごと")
            print(f"  → 処理解像度: {processing_resolution[0]}x{processing_resolution[1]}")
            print(f"  → 待機時間: {wait_key_delay}ms")
        else:
            print("\n[標準モード] デスクトップPCとして動作します。")
    except Exception as e:
    ...
    try:
        ...
        if args.fps: cap.set(cv2.CAP_PROP_FPS,args.fps)
        if not cap.isOpened(): 
            raise RuntimeError("カメラを開けませんでした。カメラが接続されているか確認してください。")
    except Exception as e:
        print(f"[エラー] カメラ初期化エラー: {e}")
        print("カメラデバイス番号を変更する場合は --camera オプションを使用してください。")
        import traceback
        traceback.print_exc()
        return
    ...
    try:
        while True:
            ...
            # フレームスキップ（ノートPC向け軽量化）
            frame_count += 1
            if frame_count % (frame_skip_interval + 1) != 0:
                continue
            
            # 解像度削減（ノートPC向け軽量化）
            if processing_resolution and frame.shape[0] > processing_resolution[1]:
        ...
        print(f"[エラー] 実行中にエラーが発生しました: {e}")
        import traceback
        traceback.print_exc()
    finally:
        cap.release(); cv2.destroyAllWindows()
        print("[情報] リソースを解放しました。")
